...
export function PanelCommonSettings({ ctx }: { ctx: PrefsContext }): JSX.Element {
  ...
  return (
    /* `bPanelSizer`, horizontal: `bPanelSizer->Add( bLeftSizer, 0, wxRIGHT, 35 )`
       (panel_common_settings_base.cpp:325) then the right column. The page is
       TWO columns with a 35px gutter, and which group sits in which is
       upstream's - Rendering Engine, Helper Applications and User Interface on
       the left, Scaling, Editing, Session and Project Backup on the right.
       Ours stacked all seven in one scrolling column. */
    <div className="ze-pref-columns">
      <div className="ze-pref-col">
        {/* `Rendering Engine` (panel_common_settings_base.cpp:21-77), and it is
            not what we had. Upstream is a wxRB_GROUP pair - `m_rbAccelerated` /
            `m_rbFallback` (:35-43) - and then ONE `Antialiasing:` wxChoice
            (:60-66). We had no engine choice at all and TWO antialiasing
            combos, one per engine, under a group named after the combos. */}
        <Group title="Rendering Engine">
          <Radio
            name="pref-rendering-engine"
            value="accelerated"
            options={[
              ['accelerated', 'Accelerated Graphics'],
              ['fallback', 'Fallback Graphics'],
            ]}
            onChange={() => {}}
            disabled
            title="The browser decides the canvas backend; there is no fallback renderer to choose."
          />
          <Sel
            label="Antialiasing:"
            value={0}
        ...
        <Group title="User Interface">
          ...
          />
          {/* `m_rbIconThemeLight` / `Dark` / `Auto`, a wxRB_GROUP run on one
              row (`:202-226`) - three radios, not a wxChoice. KiCad reaches
              for radios wherever a choice is small and all of it should be
              visible at once. */}
          <Radio
            row
            label="Icon theme:"
          ...
          />
          {/* `m_rbIconSizeSmall` / `Normal` / `Large`, the same shape
              (`:228-251`). */}
          <Radio
            row

...
/**
 * `PANEL_EESCHEMA_COLOR_SETTINGS`' `m_validLayers` and `createSwatches`
 * (`eeschema/dialogs/panel_eeschema_color_settings.cpp:52-215`), as data --
 * the schematic half of what `fpColorLayers.ts` is for the board.
 *
 * It is a module of its own and not part of the page because two things read
 * it: the page, and `prefs/theme_files.ts`, which turns a theme into the file
 * KiCad would have written. A page that owns the only copy of a mapping table
 ...
 */
export const themeByLayer = (theme: Theme): Partial<Record<SchLayerId, string>> => {

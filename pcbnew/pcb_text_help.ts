...
/**
 * The Syntax Help window's contents.
 * Counterpart: `pcbnew/pcb_text_help_md.h`, which `PCB_TEXT::ShowSyntaxHelp`
 * feeds to `ConvertMarkdown2Html` and hands to an `HTML_MESSAGE_BOX`
 * (`pcb_text.cpp:718-736`).
 *
 * [data]. That header is generated by CMake from `pcb_text_help.md` and is
 * checked in, so it is a table KiCad hardcodes: the markup rows, the text
 * variables, the footprint cross-reference syntax and the two error strings are
 * transcribed from it rather than written here. It arrives already as HTML —
 * the `.md` is markdown but the generated header holds the converted table —
 * so `ConvertMarkdown2Html` has nothing left to do to the part that matters.
 *
 * Ours is our own constant, never a message built from a board's contents,
 * which is why the dialog may render it as markup at all. See
 * `ui/dialog_html_message_box.tsx`, whose LIST path sanitises for the opposite
 * reason.
 */

/** `pcb_text_help_md.h`, verbatim. */
export const PCB_TEXT_SYNTAX_HELP = `<table>
    <tr>
        <th>Markup</th>
        <th></th>
        <th>Result</th>

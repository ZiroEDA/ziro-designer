    ...
    if (isC0C1(cset)) {
      if (canC) {
        ...
        if (i + 2 < length) {
          ...
          c += memo ? memo : cost(source, length, i + 2, cset, 0, priority, costs, modes);
        }
        if (c < minCost) {
          minCost = c;
          minMode = mode;
...
export function code128(symbol: ZintSymbol, source: readonly number[]): string {
  ...
  if (length > 170) return `Error 340: Input length ${length} too long (maximum 170)`;
  ...
  if (!values)
    return 'Error 341: Input too long, requires more than 102 symbol characters (maximum 102)';

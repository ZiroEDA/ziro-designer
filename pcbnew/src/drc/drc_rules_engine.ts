...
export function evalDrcRules(
  ...
  holeProxy = false,
  /**
   * Collects the reasoning, for the Clearance and Constraints Resolution
   * reports. Absent means the fast path: DRC itself never asks for it, and
   * upstream likewise builds these strings only when a reporter is attached.
   */
  report?: string[],
): ResolvedConstraint {
  ...
  if (localOverride !== undefined) {
    ...
    report?.push(`Local override; ${type}: ${localOverride}.`);
    return out;
  }

  const disallowKind = type === 'disallow' && a ? disallowKindFor(a, holeProxy) : undefined;
  const entries = engine.byType.get(type) ?? [];

  if (report && entries.length === 0) report.push(`No ${type} constraints defined.`);

  for (const entry of entries) {
    report?.push(`Checking ${entry.rule.name}.`);

    if (!ruleMatchesLayer(entry.rule, layer)) {
      report?.push(`Rule layer '${entry.rule.layer}' not matched; rule ignored.`);
      continue;
    }
    ...
    if (type === 'disallow' && !disallowCovers(entry.constraint.disallow ?? [], disallowKind)) {
      report?.push(entry.implicit ? 'Keepout constraint not met.' : 'Disallow constraint not met.');
      continue;
    }

    // A netclass-derived rule has no condition to test upstream either.
    if (entry.rule.condition) report?.push(`Checking rule condition '${entry.rule.condition}'.`);

    if (!conditionHolds(entry.rule.condition, a, b, out.errors)) {
      report?.push(
        entry.implicit
...
/**
 * The "why is this the answer" report behind KiCad's Clearance Resolution and
 * Constraints Resolution dialogs (`BOARD_INSPECTION_TOOL::InspectClearance`
 * and `InspectConstraints`).
 *
 * It is the same walk DRC performs, with the reasoning kept. That matters more
 * than it sounds: a report generated by a *separate* code path could disagree
 * with the markers it is meant to explain, and a user chasing a violation the
 * report says should not exist has been sent somewhere worse than nowhere.
 */
export interface ConstraintReport {
  resolved: ResolvedConstraint;
  /** One line per step, in the order the engine took them. */
  lines: string[];
}

export function reportDrcConstraint(
  engine: DrcRuleEngine,
  type: DrcConstraintType,
  a: DrcEvalItem | undefined,
  b: DrcEvalItem | undefined,
  ...
  const lines: string[] = [];
  const resolved = evalDrcRules(engine, type, a, b, layer, localOverride, false, lines);

  const { min, opt, max } = resolved.value;
  const parts: string[] = [];
  if (min !== undefined) parts.push(`min ${min}`);
  if (opt !== undefined) parts.push(`opt ${opt}`);
  if (max !== undefined) parts.push(`max ${max}`);

  lines.push(
    parts.length > 0
      ? `Resolved ${type}: ${parts.join('; ')}.`
      : `Resolved ${type}: no constraint.`,
  );

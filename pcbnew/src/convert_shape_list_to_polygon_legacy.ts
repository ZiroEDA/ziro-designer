...
/**
 * @deprecated The plain-object (`PcbShape`) form of `ConvertOutlineToPolygon` /
 * `BuildBoardPolygonOutlines`, kept only for `board_statistics.ts` until that
 * moves onto `BOARD` (#636 stage 2). The class port over `PCB_SHAPE` is
 * `convert_shape_list_to_polygon.ts`; new code uses that one.
 *

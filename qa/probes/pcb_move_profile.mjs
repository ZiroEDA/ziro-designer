...
ws.onmessage = (e) => {
  ...
  if (m.id) pending.get(m.id)(m);
    ...
    if (/Trace:|View timing|Graphics error|WebGL|Error|error|PERFDBG/.test(text)) console.log(`  [console] ${text.trim()}`);
  }
};
...
await evalJs(`window.__evs = []; for (const t of ['pointerdown','pointerup','mousedown','mouseup','click','keydown','wheel']) document.addEventListener(t, e => window.__evs.push(t + '@' + e.target.tagName + ':' + (e.button ?? e.key) + ' def=' + e.defaultPrevented), true); 'ok'`);
...
await send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: gx, y: gy });
await sleep(300);
await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: gx, y: gy, button: 'left', clickCount: 1 });
await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: gx, y: gy, button: 'left', clickCount: 1 });
await sleep(1200);
// Two candidates raise the clarification menu; its digit keys pick an entry.
const menu = await evalJs(`(() => { const lines = document.body.innerText.split(String.fromCharCode(10)).map(l => l.trim()); const i = lines.findIndex(l => /^1 \\S/.test(l)); if (i < 0) return null; return lines.slice(i, i + 12).filter(l => /^[1-9] \\S/.test(l)); })()`);
if (!menu) console.log('no clarification menu; body tail:', await evalJs(`document.body.innerText.split(String.fromCharCode(10)).filter(l => /Select All|Pad |Footprint /.test(l)).slice(0, 6).join(' | ')`));
if (menu) {
  console.log('clarification menu:', menu.join(' | '));
  const fp = menu.findIndex((l) => /Footprint/.test(l));
  const digit = String(fp >= 0 ? fp + 1 : 1);
  await send('Input.dispatchKeyEvent', { type: 'keyDown', key: digit, code: `Digit${digit}`, text: digit, windowsVirtualKeyCode: 48 + Number(digit) });
  await send('Input.dispatchKeyEvent', { type: 'keyUp', key: digit, code: `Digit${digit}`, windowsVirtualKeyCode: 48 + Number(digit) });
  await sleep(1200);
}
console.log('after CDP click:', await evalJs(`(() => { const t = document.body.innerText; const i = t.indexOf('Properties'); return t.slice(i, i + 160).split(String.fromCharCode(10)).join(' | ') + ' ... status: ' + t.slice(-160).split(String.fromCharCode(10)).join(' | '); })()`));
console.log('menus/popups:', await evalJs(`[...document.querySelectorAll('[role=menu], .ze-menu, .ze-popup, .ze-disambig')].map(e => e.className + ':' + e.innerText.slice(0, 60).split(String.fromCharCode(10)).join('/')).join(' || ')`));
console.log('events seen:', await evalJs(`window.__evs.join(' ')`));
console.log('frames so far:', frames(), 'rAF in 1 s:', await evalJs(`new Promise(r => { let n = 0; const t = performance.now(); (function f() { n++; if (performance.now() - t < 1000) requestAnimationFrame(f); else r(n); })(); })`));
console.log('element at point:', await evalJs(`(() => { const el = document.elementFromPoint(${gx}, ${gy}); return el ? el.tagName + '.' + el.className + ' ' + el.width + 'x' + el.height + ' pe=' + getComputedStyle(el).pointerEvents : 'none'; })()`));
...
await evalJs(`window.__lt = []; new PerformanceObserver((l) => { for (const e of l.getEntries()) window.__lt.push([Math.round(e.startTime), Math.round(e.duration)]); }).observe({ type: 'longtask' }); window.__t0 = performance.now(); 'ok'`);
...
await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: gx + 64, y: gy + 32, button: 'left', clickCount: 1 });
await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: gx + 64, y: gy + 32, button: 'left', clickCount: 1 });
...
console.log('long tasks since pickup (start+ms):', await evalJs(`window.__lt.map(([s, d]) => ((s - window.__t0) / 1000).toFixed(2) + 's+' + d).join(' ')`));
...
const after = consoleLines.filter(([t, s]) => t > tPick && /View timing/.test(s)).map(([t, s]) => `${((t - tPick) / 1000).toFixed(2)}s ${/view-total: ([^ ]+)/.exec(s)?.[1]}`);

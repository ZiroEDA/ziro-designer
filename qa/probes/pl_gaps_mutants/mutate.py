...
MUTANTS = {
    ...
    'M1': ('common/draw_panel_gal_grid_cursor.ts',
           '  if (j * step + origin !== 0) return null;',
           '  if (false) return null;', 1),
    'M2': ('common/draw_panel_gal_grid_cursor.ts',
           '  return (j - anchor) * dir;',
           '  return j - anchor;', 1),
    'M3': ('common/draw_panel_gal_grid_cursor.ts',
    ...
    'M4': ('common/draw_panel_gal_grid_cursor.ts',
           '        if (l === skipRow) continue;\n        const y = l * pitch;',
           '        const y = l * pitch;', 1),
    'M5': ('common/draw_panel_gal_grid_cursor.ts',
           '        if (k === skipCol) continue;\n        const x = k * pitch;\n        const sw =',
           '        const x = k * pitch;\n        const sw =', 1),
    'M6': ('common/draw_panel_gal_grid_cursor.ts',
           '|${minorW}|${skipCol},${skipRow}`;',
           '|${minorW}`;', 1),
    # ---- toolbar: a button's right-click menu ----------------------------
    'M7': ('common/tool/ui/toolbar_context_menu_registry.ts',
           "  { action: 'gridProperties' },\n  { action: 'gridOrigin' },\n];",
           "  { action: 'gridProperties' },\n];", 1),
    'M8': ('common/tool/action_toolbar_actions.ts',
           "  gridProperties: { name: 'Edit Grids...', tip: 'Edit grid definitions' },",
           "  gridProperties: { name: 'Grid Properties', tip: 'Edit grid definitions' },", 1),
    'M9': ('common/tool/action_toolbar.tsx',
           '        onContextMenu={(e) => {\n          if (opts.inPalette) return;\n          if (openButtonMenu(b.id, e)) e.preventDefault();\n        }}\n',
           '', 1),
    'M10': ('common/tool/ui/toolbar_context_menu_registry.ts',
            '      action: () => run(id),',
            '      action: () => run(actionId),', 1),
    'M11': ('common/tool/ui/toolbar_context_menu_registry.ts',
    ...
    'M15': ('common/tool/action_toolbar.tsx',

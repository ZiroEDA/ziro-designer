...
const T = (label: string, fn: () => unknown): unknown => {
  const t = performance.now();
  const r = fn();
  console.log(`  ${label.padEnd(44)} ${(performance.now() - t).toFixed(0).padStart(7)} ms`);
  return r;
};
let b = T('readBoard (parse + boardFromBOARD)', () =>
  readBoard(parse(readFileSync(f, 'utf8'))),
) as Board;
...
for (let rep = 0; rep < 3; rep++) {
  ...
  const next = T('moveBoardItems (the tool, on the view)', () =>
    moveBoardItems(b, new Set([id]), { x: 1000000, y: 0 }),
  ) as Board;
  T('commitViewToBoard (stage + write-back + Push)', () =>
    commitViewToBoard(frame, b, next, 'Move'),
  );
  const moved = fp.k!;
  b = T('boardFromBOARD (the listener re-derivation)', () => ({
    ...boardFromBOARD(b.k!, 'x', (k) => k !== moved),
    fileName: 'x',
  })) as Board;
}

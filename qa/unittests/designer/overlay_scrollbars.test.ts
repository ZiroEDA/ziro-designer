...
import {
  ...
  nextOverState,
  fadeRuns,
} from '@ziroeda/common/widgets/overlay_scrollbars.js';
...
describe('the native scrollbars are off, which is the zero-layout-cost half', () => {
  ...
  it('opts the menu out of the indicator, since GTK gives a menu arrows', () => {
    ...
    const menubar = readFileSync(
      fileURLToPath(new URL('../../../common/tool/action_menu_bar.tsx', import.meta.url)),
      'utf8',
    );
...
describe('a pane that leaves the document takes its bars with it, now', () => {
  ...
  const SRC = readFileSync(
    fileURLToPath(new URL('../../../common/widgets/overlay_scrollbars.ts', import.meta.url)),
    'utf8',
  );

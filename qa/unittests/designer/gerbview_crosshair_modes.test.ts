...
describe('the frame carries a mode, not a boolean', () => {
  ...
  it('opens on SMALL_CROSS, the GAL constructor default', () => {
    ...
    // `[^\]]*` rather than `[\s\S]*?` so the assertion survives biome deciding
    // whether the Set literal fits on one line — it pins the membership, not
    // the formatting.
    expect(VIEWER).toMatch(/DEFAULT_TOGGLES = new Set\(\[[^\]]*'crosshairSmall'/);
  });
});

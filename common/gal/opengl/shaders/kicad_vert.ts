...
/**
 * `common/gal/shaders/kicad_vert.glsl` (`BUILTIN_SHADERS::glsl_kicad_vert`),
 * in GLSL ES 3.00. The 1.20 source reads the fixed pipeline's inputs; here
 * they are what the fixed pipeline held them in:
 *
 *   gl_Vertex                     -> in vec3 a_position (VERTEX x,y,z)
 *   gl_Color                      -> in vec4 a_color    (VERTEX r,g,b,a, normalised)
 *   gl_ModelViewProjectionMatrix  -> uniform mat4 u_modelViewProjection
 *   gl_FrontColor                 -> flat out vec4 v_color (glShadeModel( GL_FLAT ))
 *   gl_TexCoord[0].st             -> out vec2 v_texCoord
 *   ftransform()                  -> u_modelViewProjection * vec4( a_position, 1 )
 *
 * Everything else is the reference line for line.
 */

export const glsl_kicad_vert = `#version 300 es
...
void computeLineCoords( bool posture, vec2 vs, vec2 vp, vec2 texcoord, vec2 dir, float lineWidth, bool endV )
{
    ...
    vec4 screenPos = u_modelViewProjection * vec4( a_position, 1.0 ) + vec4(1, 1, 0, 0);
    ...
    vec4 color = a_color;
    vec2 s = sign( vec2( u_modelViewProjection[0][0], u_modelViewProjection[1][1] ) );
...
{
    vec4 delta;
    vec4 center = roundv( u_modelViewProjection * vec4( a_position, 1.0 ) + vec4(1, 1, 0, 0), u_screenPixelSize );
    ...
    else if( vertexIndex == 3.0 )
    {
        ...
        delta = vec4( 0, 2.0 * pixelR, 0, 0 ); // ES: no int * float
    }

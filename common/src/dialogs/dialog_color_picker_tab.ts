 ...
 */
/**
 * `cfg->m_ColorPicker.default_tab` — `COMMON_SETTINGS` through `Pgm()`. The
 * app installs its settings store here at start-up, as `PGM_BASE` owns
 * `COMMON_SETTINGS` upstream; until it does, the page is remembered for the
 * session only.
 */
export interface COLOR_PICKER_TAB_STORE {
  get(): number;
  set(aIndex: number): void;
}

let sessionTab = 0;
let tabStore: COLOR_PICKER_TAB_STORE = {
  get: () => sessionTab,
  set: (i) => {
    sessionTab = i;
  },
...
/** `Pgm().GetCommonSettings()` for `m_ColorPicker`: the app's store, installed once. */
export function setColorPickerTabStore(aStore: COLOR_PICKER_TAB_STORE): void {
  tabStore = aStore;
}
...
export function loadColorPickerTab(): ColorPickerTab {
  return COLOR_PICKER_TABS[tabStore.get()] ?? 'free';
}

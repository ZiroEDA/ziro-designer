...
import { scaleForZoomFactor, zoomFactorForScale } from '../widgets/kistatusbar_format.js';
...
/* ---------------------------------------------------------------------------
   The rows themselves, and the zoom limits.

   Both branches that met here described the same control correctly and each
   carried a piece the other lacked: the exact-match reasoning above, and the
   row builder plus the limits table below. Neither side's copy of
   `ZOOM_AUTO_LABEL` or of the label function survived twice — the second
   spelling, `zoomSelectorLabel`, is gone and its callers use the name above.
   --------------------------------------------------------------------------- */
...
export function zoomChoices(
  ...
  const presets = list.map((z, i) => ({ label: zoomSelectLabel(z), preset: i + 1 }));
  ...
  return {
    choices: [auto, { label: zoomSelectLabel(zoom), preset: null }, ...presets],
    selected: 1,
  };
}

...
import { Group, Sel } from '../wx/controls.js';
...
import { gridChoiceLabel, gridEquals, type GridEntry } from '../settings/grid_settings_ui.js';
import type { EdaUnits } from '../widgets/unit_binder.js';
import type { EdaIuScale } from '../index.js';

// ---------------------------------------------------------------------------
// Folded in from grid_settings_rows.ts (09-26): upstream this is the same
...
export function PanelGridSettings({
   ...
   * no third column at all. The two lines above, as upstream writes them:
   * the actions are common's, not any one editor's.
   */
  const fastGrid1Key = KeyNameFromKeyCode(ACTIONS.gridFast1.GetHotKey());
  const fastGrid2Key = KeyNameFromKeyCode(ACTIONS.gridFast2.GetHotKey());

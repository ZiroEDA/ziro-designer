...
| status | units |
|---|---|
| **here** (13) | `dialog_assign_netclass`, `dialog_color_picker` (+ `_colors`, `_tab`), `dialog_grid_settings`, `dialog_page_settings` (the model folded in; the eeschema export pair is `DIALOG_EESCHEMA_PAGE_SETTINGS`'s, so it went there), `dialog_paste_special`, `dialog_restore_local_history`, `eda_list_dialog`, `hotkey_cycle_popup` (+ `_ui`: the model and the window, as `unit_binder` / `_ui`), `html_message_box`, `panel_embedded_files`, `panel_setup_netclasses`, `panel_setup_severities`, `panel_text_variables` |

...
| `filename_resolver` | moved 09-26, **not yet the class**: `resolvePath` is a web adaptation of `FILENAME_RESOLVER::ResolvePath` (project files by name, hosted library by URL). The class - `SetProject`, the `SEARCH_PATH` list, `:alias:` / user aliases, `ShortenPath`, `ValidateFileName`, `GetKicadPaths`, embedded files - stands on `wxFileName::FileExists`, which needs the open project mounted at an ABSOLUTE directory (KiCad's KIPRJMOD; ours is `""` because the app loads `foo.kicad_pro` by bare name). Open decision; the environment it also needs landed 09-26 |

...
| status | units |
|---|---|
| **here** (19) | `dialog_assign_netclass`, `dialog_color_picker` (+ `_colors`, `_tab`), `dialog_grid_settings`, `dialog_page_settings` (the model folded in; the eeschema export pair is `DIALOG_EESCHEMA_PAGE_SETTINGS`'s, so it went there), `dialog_paste_special`, `dialog_restore_local_history`, `eda_list_dialog`, `hotkey_cycle_popup` (+ `_ui`: the model and the window, as `unit_binder` / `_ui`), `html_message_box`, `panel_common_settings`, `panel_embedded_files`, `panel_gal_options`, `panel_grid_settings`, `panel_mouse_settings`, `panel_setup_netclasses`, `panel_setup_severities`, `panel_spacemouse`, `panel_text_variables`, `git/panel_git_repos` (the four Preferences panels read `COMMON_SETTINGS_DRAFT`, a structural slice of the book's context) |
| **to move: blocked on `COMMON_SETTINGS`** (they read `designer/src/prefs`, which is KiCad's `common/settings/common_settings`) | the Preferences panels: `panel_color_settings`, `panel_hotkeys_editor`, `panel_maintenance`, `panel_toolbar_customization`; and `dialog_hotkey_list` |

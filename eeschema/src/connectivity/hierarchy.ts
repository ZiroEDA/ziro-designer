...
import { computeNetlist, Priority, type Netlist, type NetlistOptions } from './nets.js';
...
export function computeHierarchyNetlist(
  ...
  for (const [path, graph] of graphs) bySheet.set(path, graph.netlist);
  return { bySheet, humanPaths };
}
